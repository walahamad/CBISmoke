...
public class PDP_old extends SelTestCase {
	...
	public static void addProductsToCart(LinkedHashMap<String, String> productDetails) throws Exception {
		try {
			...
			logs.debug("url key " + PDP_old.keys.url);
			logs.debug("url key value " + PDPURL);
			getDriver().get(PDPURL);

			// Apply color and check of the results reflected to PDP
			if (!((String) productDetails.get(PDP_old.keys.color)).equals("")) {
				logs.debug("selecting color: " + (String) productDetails.get(PDP_old.keys.color));
				selectColor((String) productDetails.get(PDP_old.keys.color));
			} // variant color

			// Apply size
			if (!((String) productDetails.get(PDP_old.keys.size)).equals("")) {

				logs.debug("selecting size: " + (String) productDetails.get(PDP_old.keys.size));
				selectSize((String) productDetails.get(PDP_old.keys.size));
			} // size check

			// Apply fleece
			if (!((String) productDetails.get(PDP_old.keys.fleece)).equals("")) {
				logs.debug("selecting fleece: " + (String) productDetails.get(PDP_old.keys.fleece));
				selectFleece((String) productDetails.get(PDP_old.keys.fleece));
			} // fleece check

			// Apply memory
			if (!((String) productDetails.get(PDP_old.keys.memory)).equals("")) {
				logs.debug("selecting memory: " + (String) productDetails.get(PDP_old.keys.memory));
				selectMemory((String) productDetails.get(PDP_old.keys.memory));
			} // fleece check
